...
test('correct task should be deleted from correct array', () => {
  const endState = tasksReducer(
    startState,
    removeTaskActionCreator('2', 'todolistId2'),
  );
...
test('correct task should be added to correct array', () => {
  const endState = tasksReducer(
    startState,
    addTaskActionCreator('juice', 'todolistId2'),
  );
...
test('status of correct task is changed', () => {
  const endState = tasksReducer(
    startState,
    ChangeTaskStatusActionCreator('todolistId2', '2', false),
  );
...
test('title of correct task is changed', () => {
  const endState = tasksReducer(
    startState,
    ChangeTaskTitleActionCreator('todolistId2', '1', 'New Title'),
  );
...
test('new property in new array should be added when new todolist is added', () => {
  const endState = tasksReducer(
    startState,
    addTodolistActionCreator('New todolist'),
  );
...
test('property with todolistId should be deleted', () => {
  const endState = tasksReducer(
    startState,
    removeTodolistActionCreator('todolistId2'),
  );
  const keys = Object.keys(endState);
